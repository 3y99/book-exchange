...
// ✅ Mark admin responses as viewed (clear notification)
$update = $pdo->prepare("
    UPDATE reports
    SET is_user_notified = 1
    WHERE reporter_id = ? AND admin_notes IS NOT NULL AND admin_notes <> '' AND (is_user_notified = 0 OR is_user_notified IS NULL)
");
try {
    $update->execute([$user_id]);
} catch (PDOException $e) {
    // If the column doesn’t exist yet, it won’t crash the page
}

// ✅ (Optional) If your reports table doesn’t have `is_user_notified`, add this once in phpMyAdmin or via SQL:
/// ALTER TABLE reports ADD COLUMN is_user_notified BOOLEAN DEFAULT 0 AFTER admin_notes;
?>
